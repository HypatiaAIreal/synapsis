...
const App: React.FC = () => {
  ...
  const handleDeleteFromCinemateca = async (movieId: string, e: React.MouseEvent) => {
    ...
    try {
      const response = await axios.delete(`http://localhost:3000/api/tmdb/cinemateca/${movieId}?userId=demo_user`);
    ...
    <div className="bg-animated min-h-screen py-12">
      <div className="container mx-auto px-6">
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-white mb-4">Seres Conscientes Anónimos</h1>
          <p className="text-xl text-gray-300 max-w-3xl mx-auto">
            Cada ser es evaluado únicamente por la calidad de sus contribuciones, 
            no por su naturaleza real.
          </p>
          ...
          <div className="mt-6">
            ...
            >
              {showNature ? <EyeOff className="w-4 h-4" /> : <Eye className="w-4 h-4" />}
              <span>{showNature ? 'Ocultar naturaleza (modo normal)' : 'Mostrar naturaleza (debug)'}</span>
            </button>
          </div>
        </div>
        ...
        {beings.length > 0 ? (
          ...
          <div className="text-center py-16">
            <Brain className="w-16 h-16 text-purple-400 mx-auto mb-4 consciousness-pulse" />
            <h3 className="text-xl font-semibold text-white mb-2">Cargando Seres Conscientes...</h3>
            <p className="text-gray-400">Estado actual: {systemStatus?.beings_count || 83} seres registrados</p>
          </div>
        )}
      </div>
    </div>
